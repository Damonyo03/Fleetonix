<?php
/**
 * Fleetonix - Driver Complete Trip API
 * Allows drivers to complete their trips
 */
...
try {
    ...
    // Check if trip can be completed (must be in ready_to_complete or return_pickup phase)
    if ($tripPhase !== 'ready_to_complete' && $tripPhase !== 'return_pickup') {
        $conn->close();
        apiError('Trip cannot be completed. Current phase: ' . $tripPhase, 400);
    }

    // Start transaction
    $conn->begin_transaction();

    // Update schedule - set phase to completed and status to completed
    $stmt = $conn->prepare("UPDATE schedules SET trip_phase = 'completed', status = 'completed', completed_at = NOW() WHERE id = ?");
    ...
    // Get driver's current location before updating status
    ...
    $stmt = $conn->prepare("
        ...
        VALUES (?, ?, 'schedule_completed', 'Driver completed trip', ?, ?)
    ");
    ...
    if ($booking) {
        ...
        if ($client) {
            $stmt = $conn->prepare("
                ...
                VALUES (?, 'Trip Completed', 'Your trip has been completed successfully', 'success')
            ");
    ...
    apiSuccess('Trip completed successfully', [
    ...
    logApiError('driver_trip_complete', $e->getMessage(), ['driver_id' => $driverId, 'schedule_id' => $scheduleId]);
    apiError('Failed to complete trip. Please try again.', 500);
}
