<?php
/**
 * Fleetonix - Driver Mark Dropoff API
 * Marks that the driver has dropped off the passenger
 */
...
try {
    ...
    $returnToPickup = (bool)$schedule['return_to_pickup'];
    $tripPhase = $schedule['trip_phase'] ?? 'pending';

    // Check if dropoff can be marked
    if ($tripPhase !== 'dropoff') {
        $conn->close();
        apiError('Dropoff cannot be marked. Current phase: ' . $tripPhase, 400);
    }
    ...
    if ($returnToPickup) {
        // If return trip is needed, move to return_pickup phase
        $stmt = $conn->prepare("UPDATE schedules SET trip_phase = 'return_pickup' WHERE id = ?");
        $stmt->bind_param('i', $scheduleId);
        if (!$stmt->execute()) {
            throw new Exception('Failed to update schedule');
        }
        $stmt->close();
        $newPhase = 'return_pickup';
    } else {
        // No return trip, move to ready_to_complete phase (driver needs to confirm completion)
        $stmt = $conn->prepare("UPDATE schedules SET trip_phase = 'ready_to_complete' WHERE id = ?");
        $stmt->bind_param('i', $scheduleId);
        if (!$stmt->execute()) {
            throw new Exception('Failed to update schedule');
        }
        $stmt->close();
        $newPhase = 'ready_to_complete';
    }
    ...
    $stmt = $conn->prepare("
        ...
        VALUES (?, ?, 'dropoff_completed', 'Driver dropped off passenger', ?, ?)
    ");
    ...
    if ($booking) {
        ...
        if ($client) {
            $stmt = $conn->prepare("
                ...
                VALUES (?, 'Dropoff Completed', 'Your passenger has been dropped off at the destination', 'success')
            ");
    ...
    apiSuccess('Dropoff marked successfully', [
    ...
    logApiError('driver_trip_dropoff', $e->getMessage(), ['driver_id' => $driverId, 'schedule_id' => $scheduleId]);
    apiError('Failed to mark dropoff. Please try again.', 500);
}
