<?php
/**
 * Fleetonix - Driver Mark Return Pickup API
 * Marks that the driver has picked up the passenger for return trip
 */
...
try {
    ...
    // Check if return pickup can be marked
    if ($tripPhase !== 'return_pickup') {
        $conn->close();
        apiError('Return pickup cannot be marked. Current phase: ' . $tripPhase, 400);
    }

    // Start transaction
    $conn->begin_transaction();

    // Move to ready_to_complete phase (driver needs to confirm completion)
    ...
    $stmt = $conn->prepare("
        ...
        VALUES (?, ?, 'return_pickup_completed', 'Driver completed return pickup', ?, ?)
    ");
    ...
    if ($booking) {
        ...
        if ($client) {
            $stmt = $conn->prepare("
                ...
                VALUES (?, 'Return Pickup Completed', 'Driver has completed the return pickup', 'info')
            ");
    ...
    apiSuccess('Return pickup completed. Please confirm trip completion.', [
    ...
    logApiError('driver_trip_return_pickup', $e->getMessage(), ['driver_id' => $driverId, 'schedule_id' => $scheduleId]);
    apiError('Failed to complete return pickup. Please try again.', 500);
}
