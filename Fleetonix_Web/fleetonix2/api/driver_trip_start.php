<?php
/**
 * Fleetonix - Driver Start Trip API
 * Allows drivers to start their assigned trips
 */
...
try {
    ...
    if ($schedule['status'] !== 'pending') {
        $conn->close();
        apiError('Trip cannot be started. Current status: ' . $schedule['status'], 400);
    }

    if ($schedule['booking_status'] !== 'assigned' && $schedule['booking_status'] !== 'in_progress') {
        $conn->close();
        apiError('Booking is not in a startable state', 400);
    }

    // Start transaction
    $conn->begin_transaction();

    // Update schedule - set phase to pickup
    $stmt = $conn->prepare("UPDATE schedules SET trip_phase = 'pickup', status = 'in_progress', started_at = NOW() WHERE id = ?");
    ...
    $stmt = $conn->prepare("
        ...
        VALUES (?, ?, 'schedule_started', 'Driver started trip', ?, ?)
    ");
    ...
    if ($booking) {
        ...
        if ($client) {
            $stmt = $conn->prepare("
                ...
                VALUES (?, 'Trip Started', 'Your driver has started the trip', 'info')
            ");
    ...
    apiSuccess('Trip started successfully', [
    ...
    logApiError('driver_trip_start', $e->getMessage(), ['driver_id' => $driverId, 'schedule_id' => $scheduleId]);
    apiError('Failed to start trip. Please try again.', 500);
}
